...
for filerr in list_not_total:
    ...
    for piece in read_in_chunks(y, chunk_size):
        print(f"Transferring chunk {i + 1} from file {filerr}...")
    ...
    try:
        ...
        lista = ['PBS_NPS_FFF_BRA_M_202104.CSV']
        if any(prefix in filerr for prefix in lista):
            expected.write.csv(
                f"s3://{bucket}/dev/standard/{audit}/{audit}_fact_{'_'.join(filerr.rsplit('_')[3:-2]).split('.')[0].lower()}",
                compression='gzip', mode="overwrite", header="true")
        else:
            expected.write.csv(
                f"s3://{bucket}/dev/standard/{audit}/{audit}_{'_'.join(filerr.rsplit('_')[4:]).split('.')[0].lower()}",
                compression='gzip', mode="overwrite", header="true")
